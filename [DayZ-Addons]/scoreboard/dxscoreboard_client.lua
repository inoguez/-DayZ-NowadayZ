	...
	if #scoreboardColumns ~= 0 then
		...
		if (currentTick - scoreboardTicks.lastUpdate > scoreboardTicks.updateInterval and (scoreboardToggled or scoreboardForced)) or forceScoreboardUpdate then
			...
			playerInList = false
				...
				if isElement(player) and getPlayerName(player) == getPlayerName(getLocalPlayer()) then
				playerInList = true
				end
				if isElement(player) and not getPlayerTeam( player ) or not (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) or serverInfo.forcehideteams then
				if isElement(player) and getElementData(player,"Iniciado") then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						elseif column.name == "gang" then
						local	gang = (getElementData(player,"gang") or "N/A")
								content = tostring(gang)
						else
		...
		if scoreboardDrawn then
			...
			if serverInfo.server and showServerInfo then
				dxDrawText( "Servidor: " .. serverInfo.server, topX+s(5), y, topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cServerInfo, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "left", "top", false, false, drawOverGUI )
			end
			...
			local textLength = dxGetTextWidth( "Copyright © DayZ| Powered by Hades", fontscale(rmbFont, s(0.75)), rmbFont )
			local textHeight = dxGetFontHeight( fontscale(rmbFont, s(0.75)), rmbFont )
			dxDrawText( "Copyright © DayZ| Powered by Hades", sX/2-(textLength/2), topY+scoreboardDimensions.height-textHeight-s(2), sX/2+(textLength/2), topY+scoreboardDimensions.height-s(2), cWhite, fontscale(serverInfoFont, s(0.75)), rmbFont, "left", "top", false, false, drawOverGUI )
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if content then
							...
							elseif column.name == "country" then	
								...
								if column.name == "ping" and column.name ~= "fps" then--aca para cambiar color a algo muy importante
                                    if getPlayerPing(element) >= 0 and getPlayerPing(element) <= 120 then
                                      r, g, b, a = 0, 255, 0, 255  
                                    elseif getPlayerPing(element) >= 121 and getPlayerPing(element) <= 200 then
                                      r, g, b, a = 0, 255, 0, 200
                                    elseif getPlayerPing(element) >= 201 and getPlayerPing(element) <= 500 then
                                      r, g, b, a = 255, 255, 0, 200
                                    elseif getPlayerPing(element) >= 501 then
                                      r, g, b, a = 255, 5, 0, 255    
                                    end
                                    dxDrawText( content , topX+theX+s(1),    y+s(1), topX+x+s(1+column.width),   y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ),    tocolor( r, g, b, a ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )                                    
                            else
                                    if column.name == "fps" then
                                      if getElementData(element,"FPS") >= 0 and getElementData(element,"FPS") <= 15 then
                                        r, g, b, a = 255, 0, 0, 255  
                                      elseif getElementData(element,"FPS") >= 16 and getElementData(element,"FPS") <= 30 then
                                        r, g, b, a = 255, 255, 0, 255
                                      elseif getElementData(element,"FPS") >= 31 and getElementData(element,"FPS") <= 50 then
                                        r, g, b, a = 0, 255, 0, 200
                                      elseif getElementData(element,"FPS") >= 51 then
                                        r, g, b, a = 0, 255, 0, 255    
                                      end
                                      dxDrawText( content , topX+theX+s(1),    y+s(1), topX+x+s(1+column.width),   y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ),    tocolor( r, g, b, a ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )                                   
                                    end
                                    if column.name ~= "fps" then
                                      dxDrawText( content, topX+theX,         y,      topX+x+s(column.width),     y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ),          tocolor( 128, 128, 128 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
                                    end
                                  end
