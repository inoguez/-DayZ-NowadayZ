...
function textLabels( group )
	...
	local qh = database:query("SELECT * FROM groupmembers WHERE name=?", group)
...
function getServerListGPC( )
	local newBi = {}
	local qh = database:query("SELECT groupname FROM groupsystem ORDER BY groupname ASC")
	for _, columna in ipairs( dbPoll(qh, -1) ) do
...
function getInfoGPCgRop( group )
	if isGroupExists( group ) then
		local qh = database:query("SELECT info FROM groupinfo WHERE gpname=?", group)
...
function getMembersOfGP( group )
	local newMb = {}
	local qh = database:query("SELECT * FROM groupmembers WHERE name=? ORDER BY rang DESC", group)
	for _, columna in ipairs( dbPoll( qh, -1 ) ) do
...
function findGPCPlayerG( texto, grupo )
	local NewJh = {}
	local qh = database:query( "SELECT membername, rang, status FROM groupmembers WHERE name=?", grupo )
