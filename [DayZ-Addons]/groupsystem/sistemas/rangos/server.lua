...
function deleteRanks( rankName )
	...
	local qh = dbQuery( database, "SELECT * FROM groupmembers WHERE name=?", playerGroup )
	...
	for _, columna in ipairs(poll) do
		...
		if theRank == rankName then
			dbExec( database, "UPDATE groupmembers SET rang=? WHERE memberacc=? AND name=?", "Trial", accName, playerGroup )
			groupRanks2 ( source )
		end
...
function createNewGroupRank( grupo, rankName, kick, chat, Crangos, Alianzas, apelaciones, informacion )
	...
	if grupo and rankName and kick and chat and Crangos and Alianzas and apelaciones and informacion then
		...
		dbExec( database, "INSERT INTO groupRanks ( grupo, rankName, kickP, Pchat, CRank, AAlianzas, Apelaciones, informacion) VALUES(?,?,?,?,?,?,?,?)", grupo, rankName, kick, chat, Crangos, Alianzas, apelaciones, informacion )
	end
...
function getGroupRanks( grupo )
	...
	local caC = { }
	local testQuery = dbQuery( database, "SELECT * FROM groupRanks WHERE grupo=?", grupo )
...
function getGroupRankPermisos( grupo, rankName )
	...
	if grupo and rankName then
		...
		local testQuery = dbQuery( database, "SELECT * FROM groupRanks WHERE grupo=? AND rankName=?", grupo, rankName )
		local poll      = dbPoll ( testQuery, -1 )
...
function deleteGroupRank( grupo, rank )
	dbExec( database, "DELETE FROM groupRanks WHERE grupo=? AND rankName=?", grupo, rank)
end
...
function setGroupRankPermission( grupo, rankName, permisoName, bool )
	...
	if grupo and rankName and permisoName and bool then
		...
		if self then
			dbExec( database, "UPDATE groupRanks SET "..self.."=? WHERE grupo=? AND rankName=?", tostring( bool ), grupo, rankName )
		end
...
function isRankAlreadyCreated( group, rank )
	local testQuery = dbQuery( database, "SELECT rankName FROM groupRanks WHERE grupo=?", group )
	local poll      = dbPoll ( testQuery, -1 )
