...
function getGroupConfig( grupp )
	...
	if grupp then
		...
		local qh = database:query ( "SELECT * FROM group_alianzas_configs WHERE grupo=?", tostring ( grupp ) )
		local tablita = qh:poll( - 1 )

		if #tablita == 0 then
			dbExec( database, "INSERT INTO group_alianzas_configs ( grupo, blipsmapa, chatGrupo ) VALUES( ?, ?, ? )", tostring ( grupp ), "true", "true" )
			return true, true
		else
...
function deleteAlianze( grupo, elGrupo )
	database:exec( "DELETE FROM group_alianzas WHERE grupo=? AND grupoAliado=?", tostring( grupo ), elGrupo )
	database:exec( "DELETE FROM group_alianzas WHERE grupo=? AND grupoAliado=?", tostring( elGrupo ), grupo )
...
function requestChangeGroupConfig( a, b )
	...
	if gang then
		...
		database:exec( "UPDATE group_alianzas_configs SET blipsmapa=?, chatGrupo=? WHERE grupo=?", tostring( a ), tostring( b ), tostring( gang ) )
...
function getGroupAlianzes( group )
	local qh = database:query ( "SELECT * FROM group_alianzas WHERE grupo=?", tostring ( group ) )
	local tablita = qh:poll( - 1 )
	local n = { }
	for i, v in ipairs( tablita ) do
...
function addGroupAlianze( grupo, otroGrupo )
	...
	if grupo and otroGrupo and ( grupo ~= otroGrupo ) then
		...
		if ( not esAliadoDe ( grupo, otroGrupo ) ) then
			dbExec( database, "INSERT INTO group_alianzas ( grupo, grupoAliado ) VALUES( ?, ? )", tostring ( grupo ), tostring ( otroGrupo ) )
			dbExec( database, "INSERT INTO group_alianzas ( grupo, grupoAliado ) VALUES( ?, ? )", tostring ( otroGrupo ), tostring ( grupo ) )
...
function esAliadoDe( grupo, otroGrupo )
	...
	local qh = database:query ( "SELECT * FROM group_alianzas WHERE grupo=?", tostring ( grupo ) )
	local tablita = qh:poll( - 1 )
